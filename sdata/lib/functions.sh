...
rsync_dir(){
  ...
  x mkdir -p "$(dirname ${INSTALLED_LISTFILE})"
  rsync -a --exclude='AGENTS.md' --exclude='CLAUDE.md' --exclude='CODEX.md' --exclude='PI.md' --exclude='codemap.md' --exclude='skills-lock.json' --exclude='.agents/' --exclude='.claude/' --exclude='.codex/' --exclude='.factory/' --exclude='.opencode/' --exclude='.codebase-memory/' --exclude='.impeccable/' --exclude='.pi-subagents/' --exclude='graphify-out/' --out-format='%i %n' "$1"/ "$2"/ | awk -v d="$dest" '$1 ~ /^>/{ sub(/^[^ ]+ /,""); printf d "/" $0 "\n" }' >> "${INSTALLED_LISTFILE}"
}
