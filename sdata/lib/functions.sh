...
rsync_dir(){
  ...
  x mkdir -p "$(dirname ${INSTALLED_LISTFILE})"
  rsync -a --exclude='AGENTS.md' --exclude='CLAUDE.md' --out-format='%i %n' "$1"/ "$2"/ | awk -v d="$dest" '$1 ~ /^>/{ sub(/^[^ ]+ /,""); printf d "/" $0 "\n" }' >> "${INSTALLED_LISTFILE}"
}
...
rsync_dir__sync(){
  ...
  local dest="$(realpath -se $2)"
  x mkdir -p "$(dirname ${INSTALLED_LISTFILE})"
  rsync -a --delete --exclude='AGENTS.md' --exclude='CLAUDE.md' --out-format='%i %n' "$1"/ "$2"/ | awk -v d="$dest" '$1 ~ /^>/{ sub(/^[^ ]+ /,""); printf d "/" $0 "\n" }' >> "${INSTALLED_LISTFILE}"
}
