...
- **Mascot chaos mode** (opt-in): rarely she runs across the desktop and physically wrecks it — mallet-bonks widgets so they fly with spin, knocks one out cold onto the floor where it stays lying, hurls one to a new spot, rampages through up to three in a row, or ground-slams so the bar rattles and everything wobbles. Widgets can dodge her first swing (she gets mad), flung widgets can domino into neighbours, sometimes she just trips and pretends nothing happened, and after a proper wrecking she may leave a real notification as a calling card. Widgets only keep new positions if you let her rearrange; `inir mascot tidy` puts everything back. `romp`/`tidy` IPC.
