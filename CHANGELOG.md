...
The dashboard hub, three new bar surface styles, a rebuilt YT Music on InnerTube, autostart that actually reads niri's own startup file, and the ZZZ global style going from foundation to fully shipped across the whole shell.

### Added
- **Mascot companion** (`iiMascotCompanion`): with the mascot enabled she occasionally peeks from a screen edge — sleepy at 3 AM, coffee-first in the morning — reacts to music starting, low battery, lost Wi-Fi, DND, available updates, finished gaming sessions and frantic workspace switching, and talks through translated QML speech bubbles. Strictly non-invasive: input only on her sprite, rate-limited, side peeks anchored to the bottom edge, and never over fullscreen apps, game mode, the lock screen or the session screen. Click her at your own risk (she keeps count), and don't stare. Fully customizable in Settings › Quick › Mascot (both families): test buttons to summon/dismiss her instantly, allowed screen edges, sprite size, visit interval, per-surface toggles for every static placement, and custom phrases she can say. `mascot` IPC target (`poke`/`appear`/`hide`).
- **Mascot personality upgrades**: session-long moods (`neutral`/`sleepy`/`hyper`/`snarky`/`contemplative`, seeded by time of day, re-rolled on a jittered interval, `mascotMood` IPC target) flavor her idle lines; an opt-in contextual placement makes event reactions pop up near the bar widget that caused them; a panel-sitter placement option seats her on the bar or dock instead of free-floating; she now notices wallpaper changes; and four hidden bar gestures summon her (long-press the battery, clock or update pill, tap the weather widget five times fast).
- **Mascot got smarter**: idle peeks now read the room — music playing, a pile of unread notifications, multi-day uptime, Monday mornings, Friday nights, weekends and a long-full battery each get their own pose and commentary (with real numbers in the lines), she never repeats a recent line or pose, and she sometimes comes to snoop on incoming notification popups (toggleable). Her phrase pools roughly doubled across idle, moods and reactions. New Settings › Quick › Mascot knobs: visit length (seconds) and slide-in/out speed (ms).
- **Mascot reactions are now fully yours**: every event reaction (music, low battery, updates, network drops, DND, notification snoop, wallpaper changes, screenshots, gaming sessions, workspace frenzy, and a new welcome-back peek after unlocking) gets its own on/off switch and its own pose picker in Settings › Quick › Mascot — pick a specific image per event or let her rotate the pool. Music detection got honest: browser videos posing as songs are ignored by default (she only comments when the player reports an artist, toggleable). Every trigger now draws from a pose pool instead of a fixed image, her click/stare/prank responses go through the same no-repeat memory as everything else, poses with the iNiR badge no longer render mirrored (text was backwards on right/bottom peeks), and her speech bubble matches shell tooltips in every global style.
- **Mascot got her own Settings page** (both families: Settings › Mascot): everything moved out of Quick into a dedicated page, and the per-event pose selector is now a visual thumbnail gallery — tap the current pose to expand a scrollable grid of every pose in the catalog, see exactly what you're picking, no dropdowns. Poses are edge-aware: art anchored to a screen edge (peeking, hanging, sitting) only ever appears from an edge that makes sense for it, regardless of what an override or IPC call asks for. Multi-monitor setups get a "Follow focused monitor" option, and contextual reactions now pop from wherever your bar actually is (top, bottom, or side) instead of a hardcoded top.
- **21 new poses from the maintainer's art drop**: sprite sheets cut, cleaned and wired in — a spyglass search, sticky-note overload, a purple paint roller for wallpaper changes, heavy lifting, conducting the music, watering a pixel plant, offering a golden key after you unlock, surfing the audio visualizer, a magician's top hat, an energy drink, handheld gaming, a flambé chef for updates, a skateboard trick, an alt-outfit reach, plus full-body idle attitude (hand-on-hip, unimpressed, pouting, low-angle guard, crouch inspect, victory sit).
- **Mascot art destriped**: the full catalog (86 poses + 6 animation loops) shipped with CRT-style scanline banding baked in by the generator; every asset was cleaned in place with a per-image periodic-stripe filter — same pixel-art look, no more "old monitor" stripes — and the generation pipeline now destripes automatically.
- **Mascot pose library expanded**: new use-case art for the upcoming welcome guide and the docs — tour-guide pointing poses (side/up/down), a full-body "follow me", a setup checklist, a confetti setup-complete finale, a terminal demo, bug catching, `inir doctor` stethoscope diagnostics, a rocket-assisted boot and an air-marshaller directing windows. The companion also gained three animation loops (typing, purring, chibi bounce) and mate/gossip/stretching idle poses.
- **iNiR mascot surfaces**: expressive transparent pixel-art poses now identify the Waffle About page, celebrate an empty notification center in ii and Waffle, guide the documentation landing page, hide in an empty clipboard (ii and Waffle), wave goodbye on the session screen, greet an empty AI chat and the dashboard welcome card (until a user avatar is set), teach from the cheatsheet nav rail, relax when the todo list is clear, check the rain while weather data loads, snoop around empty search results (cheatsheet, Waffle start menu), keep watch over the update overlay, chill in an empty media popup, nap on event-free calendar days, and shrug at empty wallpaper folders — all gated behind a single "Show mascot illustration" switch in About, with each surface's original presentation restored when it's off. A wide hero banner with the iNiR logotype closes the README.
- **ZZZ global style**: sixth visual worldview for Material ii (poster/console plates, square/round shape mode via `appearance.zzz.shape`, wallpaper-hue-locked surfaces). Routed through every module family, the entire common widget kit, and every settings page — bar, dock, sidebars, dashboard, overview, media controls, clipboard, region selector, alt-switcher, lock screen, OSD, cheatsheet, on-screen keyboard, shell-update banner, session screen. Pick it from Settings like any other style.
- **Dashboard hub panel** (`iiDashboard`): centered overlay with a configurable three-column widget layout — welcome greeting, clock, agenda (merges local events with ICS calendars into a day-grouped upcoming list), notifications, todo, quick notes (reuses the sidebar notepad so content stays in sync), media, weather, calendar, system usage, GitHub contributions heatmap. In-panel edit mode (per-widget move/reorder/hide, a chip tray for hidden widgets to bring back), density (comfortable/compact) and card-opacity/title-visibility appearance options, dedicated settings page, `dashboard` IPC target.
- **Bar surface styles**: islands (one cohesive capsule per bar section, edge margins aligned to compositor gaps), scenic (wallpaper gradient scrim), and frame (outlined floating frame) — alongside Classic. A layout preset gallery (Classic, Islands, Command cluster, Twin pills, Scenic minimal) bundles a surface style with the five layout zones, with a live mini-mockup and active-preset highlighting (container tint + morphing check badge) on each card.
- **Workspace edge strip** (`iiWorkspaceStrip`): hover-triggered rail with live workspace previews, window thumbnails, drag-to-reorder, and media cards. Shared by both panel families, registered as a deferred panel. `workspaceStrip` IPC target.
- **YT Music rebuilt on InnerTube**: replaced the flaky yt-dlp browse/search path (which always needed `--cookies-from-browser` and broke constantly) with a `ytmusicapi` sidecar (`scripts/innertube.py`) behind a new deferred InnerTube service — public browsing (search, home, radio, artist, album, playlist, synced lyrics via LrcLib) needs no cookies. Playback still goes through mpv/MPRIS; yt-dlp only resolves the final stream. New faithful InnerTune-style sidebar UI (Home, Search, full Player with synced lyrics and an up-next Queue sheet, Library, Account, bottom navigation), plus a frictionless device-flow login (`youtube.com/activate`) and marquee titles with drag-to-dismiss on the player.
- **Autostart manager**: settings pages (ii + waffle) and an `autostart` IPC target read/write the managed section of niri's own `~/.config/niri/config.d/50-startup.kdl` instead of duplicating that state in Config JSON.
- **News tab** in the left sidebar: Google News RSS, geo board follows the Weather-resolved city, topic boards + top stories, 15-minute cache, no API key.
- **New desktop widgets**: network, news ticker, CPU tachometer, and uptime, each with their own settings section and appearance controls in the widget manager panel. `DotGridCanvas` backs the shared drag-grid preview.
- **Theme color invert**: `appearance.colorInvert` regenerates the palette with a `--invert-hue` pass for a genuinely complementary theme instead of just recoloring in place.
- **World Clock widget toggle** was missing from Settings → Panels; added.
- **Intel GPU detection** for the resource monitor via `intel_gpu_top` (amdgpu's `gpu_busy_percent` sysfs path doesn't exist on Intel; i915/xe utilization only comes through the PMU that tool reads).
- **Flexible bar spacers**: per-instance spacer weight (1x–8x) encoded directly in the `bar.layout.*` arrays, with width/mode controls in Settings, fully wired end-to-end.
- **Region-adaptive accent helper** (`ColorUtils.adaptAccent()`): pins an accent's lightness against the wallpaper region behind it instead of the shell surface.
- **`brightness_std` from region sampling**: wallpaper region analysis now returns both mean and standard deviation of brightness, not just the mean — the mean alone lies on textured wallpapers (a region can average mid-grey while being high-contrast and unreadable).
- **Waffle `WMenu` icon column alignment**: icon-less menu items now reserve the icon column width when any sibling item carries an icon, matching the ii `BarMenu`/`ContextMenu` behavior.

### Changed
- **Settings pages slide directionally** instead of a plain fade; standalone-window and overlay modes now share one `SettingsPageRegistry` source of truth instead of two drifting page lists, search results register switch/spinbox controls, non-essential card sections start collapsed, and content width is capped to a centered readable column.
- **Settings page preloading** replaced a warm-all preloader (instantiated all 18 pages on a timer regardless of visibility, thrashing Loader create/destroy) with a current±1 window plus preload only while searching.
- **Bar settings dropped the Layout Presets grid**: each click rewrote five layout keys at once and could stall the shell; the module layout editor below already covers the same ground.
- **Media controls popup** gained player presets and a cross-slide transition between them; MPRIS controller volume/track-switch handling updated to match.
- Style-switch actions (launcher/command palette) batch through the same `setNestedValues` path Settings uses, avoiding repeated Appearance invalidations per switch.
- Picking aurora/angel as the global style no longer silently forces `appearance.transparency.enable` as a side effect — transparency stays an orthogonal preference (settings page and launcher/command-palette path both fixed to match).
- Background widget plates use a contrasting scrim (opposite of text luminance) with a minimum opacity floor instead of a same-luminance tint, so widget text stays legible over any wallpaper region. Clock's drop shadow (`colHalo`) is now a fixed dark shadow instead of `contrastColor(text)`, which glowed white behind dark text on bright wallpapers.
- Nav rails across settings and sidebars went fully background-less (button and tab-array pill), with accent-pill selection replacing grey tab tracks; the left/right sidebar empty-state blob was removed.
- ZZZ: dropped the hairline/hairlineStrong/chromeStroke border doctrine shell-wide — ZZZ separates surfaces by fill contrast, not outline strokes (also drops the left accent rail from plates and settings cards). Structural-plate saturation raised so cards carry visible wallpaper hue instead of reading flat/near-black.
- `NiriService` gains `switchToWorkspaceById` — index-based workspace focus resolves against the currently-focused output, so it can't address a workspace on a different monitor; the id-based call can (needed by the workspace strip).
- `ToolsView`'s screenshot/color-pick/window-capture tiles and the Niri Debug section now check `CompositorService.isNiri` instead of calling `niri msg` unconditionally, so Hyprland no longer hits a dead code path there.
- Docs: `ARCHITECTURE.md` panel/style counts and `docs/*.md` package/module/preset/language counts refreshed to match the live tree; a raw `qs kill` example replaced with the `inir` CLI wrapper.

### Fixed
- **Reddit sidebar panel removed**: the API returns 403 without auth, and OAuth required every user to register their own app — too much friction for a sidebar feed. Removed the service, module, config schema, settings UI, and orphan translation keys; migration drops the now-unused `sidebar.reddit` config key.
- **Dark/light choice not persisted through regeneration** *(#178, partial)*: the explicit dark/light toggle is now written to config before `switchwall.sh` runs and honored from `colors.json`'s `darkmode` field when present, instead of always being inferred from background luminance. A gap remains for presets whose generated JSON omits `darkmode` entirely — that authoritative-toggle fix lives on an unmerged branch, not in this release.
- **Wi-Fi indicator showed disconnected on non-English locales**: `nmcli` localizes the device `STATE` field (`connected` → `conectado`, etc.), but device-state parsing matched English keywords without forcing `LANG=C`. No branch matched on non-English desktops, so the Wi-Fi indicator and network panel showed disconnected with an empty list despite an active connection.
- **Overview search text clipping** *(#179)*: toolbar text fields now size from content height and center text vertically, so scaled fonts can't clip in the launcher.
- **Screen recording ignored the configured acceleration mode** *(#181)*: legacy `cpu`/`gpu` config values weren't normalized, and the safe fallback path didn't force `libx264` even when available.
- **Media widget crash on PipeWire stream binding loop**: `playerStreamNode` read `Audio.outputAppNodes` reactively while a `PwObjectTracker` held the resolved node, creating a cycle that could SIGSEGV the bar when a new stream (e.g. MPD's PipeWire output) appeared. Now recomputed imperatively on node-list/player change instead of via a live binding.
- **`workspaceStrip` panel didn't seed on fresh installs or family switches**: its loader entry existed in `ShellIiPanels.qml` but was missing from `ShellWafflePanels.qml` and from `shell.qml`'s panel-family list; the CPU Tachometer and News Ticker widgets were also missing translation keys.
- **IPC docs source-of-truth misclassification**: `background` is implemented outside both family subtrees and loaded by both shells, but its `docs/IPC.md` entry could physically sit under the waffle heading, so the doc-generation tool's family guess lost to heading position. An explicit shared-target set now wins first.
- **`flake.nix` shipped a broken NixOS/Home Manager payload** *(#186)*: root `*.qml` files weren't copied into the Nix payload, the wrapped kirigami package ships no QML (needed `kirigami.passthru.unwrapped` instead), and `patchShebangs` was mangling Python scripts.
- **Lock screen crash on screen sleep/disconnect**: null `modelData` on some monitor-state transitions threw a `TypeError`; delegate props and `onShouldPushChanged` are now null-guarded, and the `lock` IPC target now honors `lock.useHyprlock`.
- **`TargetRegion.qml` threw a `ReferenceError` on every region-selector open**: missing `ColorUtils.applyAlpha` functions import.
- **Angel-style theme editor card never activated**: its `Loader` was missing an `id`.
- YT Music no longer spawns its five background processes for users who never enabled the feature; initialization is now lazy, on first enable.
- Hotspot/WARP status pollers in quick toggles are gated on panel visibility instead of running every 5 seconds for the entire session regardless of whether the sidebar/action center is open.
- `AbstractBackgroundWidget` free-placement mode had a self-referential binding loop on its overflow clamp (two `Binding`s reading `root.x`/`root.y` while also writing them); replaced with imperative clamping in the existing geometry debounce. `WidgetSurface`'s ZZZ border toggle was a no-op (`ZzzPlate` always filled regardless of `showBorder`) — fill and stroke now gate independently.
- `WidgetManagerPanel` threw a `ReferenceError` on `GlobalStates` from a missing `import qs`; edit-panel slider labels now align to a common column via `Layout.preferredWidth`.
- AI chat and command-palette overlay leaked stale QML objects on message/mask teardown (`removeMessage()`/`clearMessages()` only dropped the map reference instead of calling `destroy()`; the overlay's clickable-region mask rebuild leaked the previous `Region` batch on every rebuild).
- Media-controls background widget rendered empty: `ScriptModel.values` expects a `QVariantList`, but `MprisController.displayPlayers` is a typed `list<MprisPlayer>` (`QQmlListReference`) that can't be assigned directly — the assignment silently failed. Spread into a plain JS array; also bound the created player popup's width/height so it tracks the widget instead of staying at 0.
- `RoundCorner`'s corner mask is now kept opaque black without an `Appearance` singleton import, avoiding a dependency that mask component shouldn't need.
- Bar mic icon lost its material `colOnLayer2` fallback color during the zzz token pass; restored. `BarContent`'s `colText` color `Behavior` wasn't gated on `animationsEnabled`.
- Bar and dock glass-wash masks previously defaulted to a different corner radius than the plate they washed, leaving an unwashed wedge poking out past the cut corner on ZZZ — masks now pass the same chamfer the visible plate cuts. `ZzzGlassWash`'s own default mask now follows the active silhouette (round/chamfer) instead of always defaulting to a bottom-right chamfer; the overview search widget's redundant paper fill (now handled by the wash) was dropped.
- Nav rail clicks were sometimes eaten by an interactive `Flickable` that didn't need to scroll; now only interactive when content actually overflows. Bottom widget group press feedback (glyph dip) restored on the bgless button style.
- Settings standalone window had a stray top-border "tape" strip on ZZZ that the overlay didn't; removed for parity. `QuickConfig`/`ZzzStyleEditor` picked up matching zzz branches.
- `SlidersSection` in the control panel duplicated the brightness/volume/mic slider rows inline instead of reusing the shared `QuickSliders` component; now deduplicated.
- **ZZZ style — a long tail of style-pass bugs found while routing every module through zzz-aware tokens**: settings card fill matched the page background with no visible separation (now a step darker); alt-switcher's decorative plate anchored to an item in a *different* top-level `PanelWindow` (silently failing — "Cannot anchor to an item that isn't a parent or sibling") and never actually drew, moved to sibling position inside the correct panel; `NotificationItem` teleported its radius on style switch (missing `Behavior`); dashboard had an undefined-bool `WARN`; MenuButton's color `Behavior` wasn't gated on `animationsEnabled`; settings body labels and non-toggled setting icons used a color that went unreadable/near-black on zzz cards (now `colOnSurface`); settings card header icons (sticker/secondary) were missing zzz branches entirely; workspace strip cards used a hardcoded `#000000` scrim fallback instead of the `colScrim` token; a diagonal-pattern widget had a per-stripe color `Behavior` generating unnecessary idle binding churn.
