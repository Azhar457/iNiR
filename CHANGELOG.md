...
- **Mascot chaos mode** (opt-in): rarely she runs across the desktop and physically messes with it — mallet-bonks a widget so it goes flying with spin, hurls one to a new spot, or ground-slams so the bar rattles and everything wobbles. Widgets only keep new positions if you let her rearrange; `inir mascot tidy` puts every displaced widget back. `romp`/`tidy` IPC.
