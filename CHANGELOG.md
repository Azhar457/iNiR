...
### Added
- **Mascot companion "Kira"** (`iiMascotCompanion`): a retro pixel-art cat girl who peeks from screen edges and reacts to shell events — music, battery, updates, network, DND, notifications, wallpaper changes, screenshots, gaming sessions, workspace frenzy and screen unlock. 130+ poses with no-repeat rotation, session moods, time-of-day flavor and context-aware commentary. Strictly non-invasive: never over fullscreen apps, games, the lock or session screens, shared cooldown between reactions. `mascot` and `mascotMood` IPC targets.
- **Dedicated Mascot settings page** (both families): every reaction has its own toggle and a visual thumbnail gallery to pick its pose; also edges, size, visit interval/length, custom phrases, panel-sitter and contextual placement, and a follow-focused-monitor option. Music detection ignores artist-less MPRIS players (browser videos) by default.
- **Mascot across the shell**: ~18 static surfaces (About, empty states, session screen, boot greeting, update overlay, docs landing) behind one switch, four hidden bar gestures that summon her, and a desktop widget with its own pose picker. Everything ships **disabled by default**; her ~180-asset art pack is an optional `./setup` Extras download (~20 MiB) so nobody pays for what they don't use.
- **Arrange settings** (Settings › Arrange): rearrange the settings sidebar with a pick-and-place editor — tap a page or group to lift it, tap where it goes; rename groups, create new ones, and drop pages into a Hidden zone to get them out of the sidebar entirely (search still finds them). One click resets to defaults.
- **Mascot desktop widget upgrades**: clicking her escalates through the same reaction ladder as the live companion (annoyed → pats → rage, with reverting pose swaps), and a custom-image field accepts any picture or GIF in place of the catalog pose.
- **Chaos got three more tricks**: she can steal a widget and waddle off-screen with it (tidy brings it back), run a full parkour line hurdling every widget without touching one, or stage a dramatic inspection that ends in nothing at all. She picks victims by how much screen they hog, hits land with little sound-theme thuds (toggleable), and after wrecking things she always comes back minutes later — in a random form, with different lines — to clean up her own mess.
- **She feels alive now**: real run-cycle animation frames (plus the comedic hop-run, tiptoes, and a right-facing dash, rotated randomly), per-outing speed that ranges from a stroll to full zoomies, and on long runs she sometimes stops halfway, turns, and just looks at you before carrying on. In chase mode she genuinely hunts your cursor — stalking low when far, crouching in striking range, lunging when close.
- **Mascot chaos mode** (opt-in): rarely she runs across the desktop — real 4-frame run cycle, sometimes sneaking on tiptoes — and physically wrecks it: mallet-swings and flying-kicks widgets so they fly with spin, knocks one out cold onto the floor where it stays lying, carries one off to a new spot, rampages through up to three in a row, drop-kicks the bar or dock, or ground-slams so everything rattles. Widgets can dodge her first swing (she gets mad), flung widgets domino into neighbours, sometimes she trips and pretends nothing happened, and after a proper wrecking she may leave a real notification as a calling card. Poke her mid-rampage at your own risk — it might turn into a **chase game** where she hunts your mouse: every click is a spot she pounces on, and clicking her means you caught her. Widgets only keep new positions if you let her rearrange; `inir mascot tidy` puts everything back. `romp`/`chase`/`tidy` IPC. 11 new action poses generated for the feature (run cycle, mallet swing, ground slam, flying kick, crate carry, hurl, trip, dust-off, tiptoe, victory stomp).
...
- **Bar surface styles**: islands, scenic and frame join Classic, plus a layout preset gallery with live mini-mockups.
